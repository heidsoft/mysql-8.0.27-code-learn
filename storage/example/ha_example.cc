 ...
 * 初始化示例函数
 * 
 * 该函数用于初始化示例手柄（handlerton）结构体，设置其状态、创建处理器、标志位等。
 * 
 * @param p 指向handlerton结构体的指针，该结构体将被初始化。
 * @return 总是返回0，表示初始化成功。
 */
static int example_init_func(void *p) {
  DBUG_TRACE; // 启用调试跟踪

  // 使用指向的p初始化example_hton结构体
  example_hton = (handlerton *)p;
  example_hton->state = SHOW_OPTION_YES; // 设置状态为SHOW_OPTION_YES
  example_hton->create = example_create_handler; // 设置创建处理器函数
  example_hton->flags = HTON_CAN_RECREATE; // 设置标志位为HTON_CAN_RECREATE
  example_hton->is_supported_system_table = example_is_supported_system_table; // 设置系统表支持函数
